...
describe 'Suggestion', ->
  ...
  describe '#initialize', ->
    ...
    it 'creates a unique id for the suggestion dom element', ->
      expect( suggestion.id ).toEqual( '1-swiftmate-suggestion' )
  ...
  describe '#select', ->
    ...
    it 'calls the provided callback with the term, data, type, index, and dom id', ->
      ...
      expect( callback ).toHaveBeenCalledWith( 'mitch crowe', {}, 'people', 1, '1-swiftmate-suggestion' )
  ...
  describe '#render', ->
    ...
    it 'sets the class to "swiftmate-suggestion"', ->
      expect( $(suggestion.render( callback )) ).toHaveClass( 'swiftmate-suggestion' )
