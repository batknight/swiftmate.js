...
describe 'SuggestionCollection', ->
  ...
  describe '#render', ->
    ...
    it 'renders all of the suggestions', ->
      expect( $('.swiftmate-suggestion', $(rendered)).length ).toEqual( 6 )
    ...
    it 'renders the suggestions for each type inside a ul', ->
      ...
      typeLists.each ->
        expect( $('.swiftmate-suggestion', $(this)).length ).toEqual( 3 )

    it 'renders a list item container for each type', ->
      expect( $(rendered).filter('li.swiftmate-type-container').length ).toEqual( 2 )

    it 'renders each type as a div with a class of "swiftmate-type"', ->
